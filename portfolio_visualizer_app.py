...
Version Corrigée - Gestion améliorée des erreurs yfinance
"""
...
def download_data(tickers, start_date, end_date):
    ...
    try:
        # Convertit les dates en format string pour yfinance
        ...
        if len(tickers) == 1:
            data = yf.download(tickers[0], start=start_str, end=end_str, progress=False)
            if data.empty:
                return None
            ...
            if data.empty:
                return None
            # Extrait uniquement les prix ajustés
...
def simulate_portfolio(prices_df, allocations, initial_capital, contribution_amount, 
    ...
    Retourne:
    - Series avec valeur totale du portefeuille par date
    """
...
if run_simulation:
    with st.spinner("Téléchargement des données..."):
        ...
        if prices_data is None or prices_data.empty:
            st.error("❌ Impossible de récupérer les données. Vérifiez:")
            st.error("• Les tickers sont corrects (ex: AAPL, MSFT, SPY)")
            st.error("• La période est valide (pas trop récente ou ancienne)")
            st.error("• Votre connexion internet fonctionne")
            st.stop()
        ...
        if missing_tickers:
            ...
            st.info("💡 Vérifiez l'orthographe sur Yahoo Finance (ex: AAPL pas APPL)")
            st.stop()
