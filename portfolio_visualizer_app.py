...
Version Finale Complète avec Synchronisation des Dates + Navigation Historique Étendue
"""
...
def download_data_with_sync(tickers, start_date, end_date):
    """
    Télécharge les données avec synchronisation automatique des dates.
    Retourne: (DataFrame des prix, date de début effective, dict des dates individuelles)
    """
    try:
        # Convertit en string
        ...
        first_dates = {}  # Pour tracker la première date de chaque ticker

        # Télécharge ticker par ticker
        for ticker in tickers:
            try:
                df = yf.download(ticker, start=start_str, end=end_str, progress=False, auto_adjust=True)

                if df.empty:
                    st.warning(f"⚠️ Aucune donnée pour {ticker}")
                    continue

                # Utilise Close (qui est déjà ajusté avec auto_adjust=True)
                if 'Close' in df.columns:
                    all_prices[ticker] = df['Close']
                    # Enregistre la première date disponible pour ce ticker
                    first_dates[ticker] = df.index[0]
                else:
        ...
        # Vérifie qu'on a au moins un ticker valide
        if all_prices.empty:
            return None, None, None

        # ===== SYNCHRONISATION DES DATES =====
        # Trouve la date de début commune (la plus récente des premières dates)
        common_start_date = max(first_dates.values())

        # Filtre le DataFrame à partir de cette date commune
        all_prices = all_prices[all_prices.index >= common_start_date]

        # Supprime les lignes avec des NaN (au cas où)
        all_prices = all_prices.dropna()

        if all_prices.empty:
            return None, None, None

        return all_prices, common_start_date, first_dates
...
def calculate_individual_metrics(prices_df, tickers):
...
with st.sidebar:
    ...
    with col1:
        start_date = st.date_input(
            ...
            min_value=datetime(1970, 1, 1),  # ← CORRECTION: Navigation depuis 1970
            max_value=datetime.now(),
            help="La date effective sera ajustée automatiquement selon la disponibilité des données"
        )
    with col2:
    ...
    # ========================================================================
    # 4. ALLOCATION AMÉLIORÉE
    # ========================================================================
    st.subheader("4. Allocation")

    # Option de répartition égale
    equal_allocation = st.checkbox(
        "⚖️ Répartition égale automatique",
        value=False,
        help="Distribue le capital équitablement entre tous les actifs"
    )
    ...
    if equal_allocation:
        ...
        for ticker in tickers:
            allocations[ticker] = equal_weight / 100
            total_weight += equal_weight

        # Affiche les poids égaux (en lecture seule)
        st.info(f"Chaque actif : {equal_weight:.2f}%")
        for ticker in tickers:
            st.text(f"{ticker}: {equal_weight:.2f}%")
    else:
        # Saisie manuelle avec number_input
        st.markdown("Entrez les pourcentages manuellement:")
        ...
        for ticker in tickers:
            weight = st.number_input(
                ...
                help="Entrez directement le pourcentage souhaité"
            )
    ...
    if abs(total_weight - 100) > 0.01:
        st.error(f"⚠️ La somme des poids est {total_weight:.2f}%. Elle doit être égale à 100%.")
        st.stop()
    else:
        st.success(f"✅ Allocation totale: {total_weight:.2f}%")
...
if run_simulation:
    with st.spinner("Téléchargement et synchronisation des données..."):
        # Télécharge les données + SPY pour le benchmark avec synchronisation
        all_tickers = list(set(tickers + ['SPY']))
        prices_data, common_start_date, first_dates = download_data_with_sync(all_tickers, start_date, end_date)

        if prices_data is None or prices_data.empty:
            st.error("❌ Impossible de récupérer les données.")
            st.info("💡 Essayez avec des dates plus anciennes (ex: 2020-2024)")
            st.stop()
        ...
        if missing_tickers:
            ...
            st.info("💡 Vérifiez l'orthographe sur finance.yahoo.com")
            st.stop()

        # Vérifie SPY
        if 'SPY' not in prices_data.columns:
            st.error("❌ Impossible de charger le benchmark SPY")
            st.stop()
    ...
    st.info(f"""
    📅 **Synchronisation des dates effectuée**

    La simulation commence à **{common_start_date.strftime('%d/%m/%Y')}** car c'est la première date où tous les actifs sélectionnés ont des données disponibles.

    **Premières dates de cotation par actif :**
    {chr(10).join([f"• {ticker}: {date.strftime('%d/%m/%Y')}" for ticker, date in sorted(first_dates.items())])}
    """)

    if common_start_date.date() > start_date:
        st.warning(f"⚠️ La date de début souhaitée ({start_date.strftime('%d/%m/%Y')}) a été ajustée à {common_start_date.strftime('%d/%m/%Y')} pour garantir la cohérence des données.")
    ...
    ax.plot(portfolio_norm.index, portfolio_norm.values, label='Votre Portefeuille', linewidth=2.5, color='#2E86AB')
    ...
    # ========================================================================
    # SECTION: PERFORMANCE INDIVIDUELLE DES ACTIFS
    # ========================================================================

    st.markdown("## 🎯 Performance Individuelle des Actifs")

    # Graphique comparatif des actifs individuels
    fig_individual, ax_individual = plt.subplots(figsize=(14, 6))

    colors = ['#E63946', '#F1A208', '#06FFA5', '#118AB2', '#073B4C', '#8338EC']

    for i, ticker in enumerate(tickers):
        prices_norm = (prices_data[ticker] / prices_data[ticker].iloc[0]) * 100
        ax_individual.plot(
            prices_norm.index, 
            prices_norm.values, 
            label=ticker, 
    ...
    # Ajoute aussi le benchmark pour comparaison
    spy_norm_individual = (prices_data['SPY'] / prices_data['SPY'].iloc[0]) * 100
    ax_individual.plot(
        spy_norm_individual.index, 
        spy_norm_individual.values, 
        label='SPY (Benchmark)', 
        linewidth=2, 
        color='#999999', 
        linestyle='--',
    ...
    ax_individual.set_xlabel('Date', fontsize=12)
    ax_individual.set_ylabel('Valeur Normalisée (Base 100)', fontsize=12)
    ax_individual.set_title('Comparaison de la Performance de Chaque Actif', fontsize=14, fontweight='bold')
    ...
    st.markdown("### 📊 Métriques Détaillées par Actif")

    individual_metrics = calculate_individual_metrics(prices_data, tickers)

    # Style le DataFrame pour un meilleur affichage
    st.dataframe(
        individual_metrics.style.format({
            'Rendement Total (%)': '{:.2f}%',
            'CAGR (%)': '{:.2f}%',
            'Volatilité (%)': '{:.2f}%',
            'Max Drawdown (%)': '{:.2f}%',
            'Sharpe Ratio': '{:.2f}'
        }).background_gradient(cmap='RdYlGn', subset=['CAGR (%)', 'Sharpe Ratio']),
        use_container_width=True
    )

    # ========================================================================
    # SECTION: KPIs DU PORTEFEUILLE GLOBAL
    # ========================================================================

    st.markdown("## 📊 Métriques du Portefeuille Global")
    ...
    - 🎯 **Performance individuelle** de chaque actif
    - ⚙️ **Allocation flexible**: saisie manuelle ou répartition égale
    - 📅 **Synchronisation automatique** des dates de cotation
    - 🕰️ **Navigation historique** jusqu'en 1970
    """)
